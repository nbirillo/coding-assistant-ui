...
import org.jetbrains.research.ml.codingAssistant.ui.MainController
...
abstract class PaneControllerManager<T : PaneController> {
    ...
    fun createContent(project: Project, scale: Double): JFXPanel {
        logger.info("${Plugin.PLUGIN_NAME}:${this::class.simpleName} create content, " +
        ...
        Platform.runLater {
            ...
            logger.info("${Plugin.PLUGIN_NAME}:${this::class.simpleName} create controller, " +
                    "current thread is ${Thread.currentThread().name}")
...
//            })
            ...
            logger.info("${Plugin.PLUGIN_NAME}:${this::class.simpleName} set controller, " +
                    "current thread is ${Thread.currentThread().name}")
    ...
    fun setVisible(isVisible: Boolean) {
        logger.info("${Plugin.PLUGIN_NAME}:${this::class.simpleName} set visible $isVisible, " +
                "current thread is ${Thread.currentThread().name}")
