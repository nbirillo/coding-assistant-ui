package org.jetbrains.research.ml.codingAssistant.tracking.dialog

import org.jetbrains.research.ml.codingAssistant.models.Task
import org.jetbrains.research.ml.codingAssistant.models.TaskSolvingErrorText
import org.jetbrains.research.ml.codingAssistant.server.PluginServer
import org.jetbrains.research.ml.codingAssistant.ui.panes.util.LowerCaseFormatter
...
class ReadOnlyDialogWrapper(private val task: Task) : CustomDialogWrapper() {
    ...
    private val defaultTaskSolvingErrorText = TaskSolvingErrorText(
        "Task solving",
        "You cannot edit this file until you choose the task. " +
            "To start or continue solving the %s task, choose it on the task choosing panel " +
            "in the CodingAssistant plugin and press %s."
    )
    ...
    override fun createMessage(): String {
        ...
        return """<html>${
            java.lang.String.format(
                ...
                PluginServer
                    .paneText?.taskChoosingPane?.get(currentLanguage)?.startSolving?.let { formatter.format(it) }
            )
        }</html>"""
    }
