...
abstract class QueryExecutor {
    ...
    private val client: OkHttpClient by lazy {
        logger.info(
            "${Plugin.PLUGIN_NAME}: init the server. API base url is $baseUrl. " +
    ...
    protected fun executeQuery(request: Request): Response? {
        ...
        fun executeQueryHelper(): Response? {
            ...
            try {
                curCountAttempts++
                logger.info(
                    "${Plugin.PLUGIN_NAME}: An attempt $curCountAttempts of execute " +
                            "the query ${request.method} ${request.url} has been started"
                )
                ...
                if (response.isSuccessful) {
                    logger.info(
                        "${Plugin.PLUGIN_NAME}: The query ${request.method} ${request.url} " +
