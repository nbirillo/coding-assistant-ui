...
internal object MainController {
    ...
    init {
        ...
        subscribe(
            ServerConnectionNotifier.SERVER_CONNECTION_TOPIC,
            object : ServerConnectionNotifier {
                override fun accept(connection: ServerConnectionResult) {
                    logger.info(
                        "${Plugin.PLUGIN_NAME} MainController, " +
                            "server connection topic $connection, current thread is ${Thread.currentThread().name}"
                    )
                    ApplicationManager.getApplication().invokeLater {
                        logger.info(
                            "${Plugin.PLUGIN_NAME} MainController, " +
                                "server connection topic $connection in application block, " +
...
//                            Todo: what pane to show if task is null? ErrorController with outdated refresh action?
    ...
    fun createContent(project: Project): JComponent {
        logger.info(
            "${Plugin.PLUGIN_NAME} MainController create content, " +
                "current thread is ${Thread.currentThread().name}"
        )
    ...
    data class Content(
        ...
        fun updatePanesToCreate() {
            ...
            if (canCreateContentPanes.isNotEmpty()) {
                ...
                Platform.runLater {
                    logger.info(
                        "${Plugin.PLUGIN_NAME} updatePanesToCreate in platform block, " +
                            "current thread is ${Thread.currentThread().name}"
                    )
